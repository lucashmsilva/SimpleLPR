...
from os.path import splitext, join, realpath, dirname, isfile
...
def load_models(models_dir="../models"):
    model_folders = listdir(models_dir)
    models = []
    for folder in model_folders:
        algs.append(folder)
        model_file = listdir(join(models_dir, folder))
        models.append(joblib.load(join(models_dir, folder, model_file[0])))
    return models
...
def compute_preditcion(sorted_strigs, plate_label, verbose=0):
    ...
    alg = 0
    if verbose == 1:
        print('{:<22s} {:<5s}'.format("LABEL:", plate_label))
    ...
    for model_name, string, ac in sorted_by_model:
        ...
        if verbose == 1:
            print('{:<10s} {:<10s} {:<5s}'.format(
                model_name, "PREDICTION:", string), opt)
        alg += 1
...
def test_all():
    ...
    round(seg_rate, 2)
    output.append(">>> SEGMENTATION \"ACCURACY\": " +
                  str(right_seg)+'/'+str(n_plates)+'\n')
    ...
    rate = round(rate, 2)
    output.append("\n>>> GENERAL \"ACCURACY\" USING ALL AVAILABLE MODELS: " +
                  str(cont_100_geral[0])+'/'+str(right_seg)+'\n')
    ...
    for alg in algs:
        ...
        output.append("\t>>> GENERAL \"ACCURACY\": " +
                      str(cont_100[alg])+"/"+str(right_seg)+'\n')
        rate = 1.0*cont_100[alg]/right_seg
        rate = round(rate, 2)
        output.append('\t\t' + str(rate) + "\n")

        output.append("\t>>> GENERAL AVERAGE \"ACCURACY\" WITH ALL PLATES: " +
                      str(round(ac_geral[alg], 2))+"/"+str(right_seg)+'\n')
